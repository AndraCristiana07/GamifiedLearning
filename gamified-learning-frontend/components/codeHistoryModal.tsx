...
export default function CodeHistoryModal({
    ...
    const [selected, setSelected] = useState<number[]>([]); // selected codes from history for diff
    return (
        <><CodeDiffModal
            open={diffOpen}
            oldCode={oldCode}
            newCode={newCode}
            language={language}
            onClose={() => setDiffOpen(false)} /><AnimatePresence>
                {open && (
                    <motion.div
                        className="fixed bg-black inset-0 flex items-center justify-center z-50 ">
                        <motion.div
                            className="bg-gray-900 p-10 inset-0 flex opacity-100 rounded-xl max-h-[80vh] overflow-y-auto"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}>
                            <motion.div
                                className="bg-gray-900/75 p-6 rounded-xl text-center"
                                initial={{ opacity: 0, scale: 0.8 }}
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.8 }}>
                                <h2 className="text-xl font-semibold text-white mb-4">Code History</h2>
                                {selected.length === 2 && (
                                    <button className="mb-4 bg-green-600 hover:bg-green-500 rounded px-4 py-2 text-white cursor-pointer"
                                        onClick={() => {
                                            const first = history[selected[0]].code;
                                            const second = history[selected[1]].code;
                                    ...
                                    {history.map((entry, index) => (
                                        <div key={index} className="bg-gray-800 p-4 rounded text-left break-inside-avoid">
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <button className="ml-2 bg-blue-700 px-2 py-1 rounded text-sm font-semibold cursor-pointer"
                                                    onClick={() => {
                                                        setSelected(prev => {
                                                    ...
                                                    {selected.includes(index) ? "Deselect" : "Select"}
                                                </button>
